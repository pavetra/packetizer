module.exports = function ( options, callback )
{
	...
	// Options
	var // Directory to import packages into
		// Absolute or relative to current package root
		targetPath = options.targetPath || './',
		// Import packages from "dependencies" list of package.json
		includeDependencies = lo.isUndefined(options.includeDependencies) ? true : options.includeDependencies,
		// Import packages from "devDependencies" list of package.json
		includeDevDependencies = lo.isUndefined(options.includeDevDependencies) ? true : options.includeDevDependencies,
		// Only import specified packages
		onlyThese = options.onlyThese || [],
		// Only import specified packages and ALL their dependencies
		onlyTheseAndDependent = options.onlyTheseAndDependent || [],
		// Only import packages with names starting with the specified prefix
		packagePrefix = options.packagePrefix || "",
		// Load dependent packages recursively
		recurseDependencies = lo.isUndefined(options.recurseDependencies) ? true : options.recurseDependencies,
		// Fail if there are two packages with different versions in a dependency tree
		// Otherwise will import the one with the highest version
		strictVersionCheck = lo.isUndefined(options.strictVersionCheck) ? true : options.strictVersionCheck,
		...
		verboseLogging = lo.isUndefined(options.verboseLogging) ? false : options.verboseLogging,
		// Custom hooks
		hooks = options.hooks || {};
		...
		try
		{
			...
			if (wouldRegister)
			{
				packages[packageJson.name] = {
					...
					path: pkgPath,
					absolutePath: path.resolve(pkgPath),
					version: packageJson.version,
					exportsPath: packageJson.packetizer.exportsPath || './',
					exportsAbsolutePath: path.resolve(pkgPath, packageJson.packetizer.exportsPath || ''),
	...
	var invokeHooks = function ( )
	{
		...
		var context = {
			...
			targetAbsolutePath: path.resolve(targetPath),
		...
		lo.forEach(packages, function ( pkg )
		{
			...
			context.packagePromises[pkg.name] = new Promise(function ( resolve, reject )
			{
				// Make it durable
				var pkgName = pkg.name;

				// This is a tricky bit...
				// We can't just then an .all promise because packagePromises[pkgName] is empty at the moment,
			...
			lo.forEach(pkg.hooks, function ( arg, hook )
			{
				...
				// Check for the hook function existence
				if (!lo.isFunction(hooks[hook])) throw new Error("Hook " + hook + " is not defined");

				// Have we invoked the hook before?
				if (lo.isUndefined(context.hookPromises[hook]))
				{
