module.exports = function ( options, callback )
{
	...
	var loadPackages = function ( pkgPath ) { return new Promise(function ( resolveLoad, rejectLoad )
	{
		...
		try
		{
			...
			{
				log("skipped, missing exports.json", {symbol: '-', pkg: {path: pkgPath, version: packageJson.version}, verbose: true});
			}
	...
	var invokeHooks = function ( )
	{
		...
		lo.forEach(packages, function ( pkg )
		{
			...
			context.pkgPromises[pkg.name] = new Promise(function ( resolve, reject )
			{
				...
				invokedPromise.then(function ( ) { Promise.all(pkgPromises[pkgName]).then(resolve, reject); }, reject);
			});

			lo.forEach(pkg.exports, function ( arg, hook )
			{
				...
				if ('undefined' == typeof context.hookPromises[hook])
				{
					...
					context.hookPromises[hook] = new Promise(function ( resolve, reject )
					{
						...
						var hookName = hook;
						invokedPromise.then(function ( ) { Promise.all(hookPromises[hookName]).then(resolve, reject); }, reject);
					});

					// Call the hook function
					// It MUST return a package hook function
					pkgHooks[hook] = hooks[hook](context);
				}

				// Call the package hook function
				// It MUST return a promise
				var promise = pkgHooks[hook](pkg, arg);
